...
雪球 1件の合計33件の閉じた操作を提供します。17件の OpenCLI
...
`OPENCLI_CONFIG_DIR` 経由で参照します。雪球は信頼済みホストの1回限りの
session、Exa Web と Code は別々の固定 MCP method を使用します。各操作は
固定された backend contract
