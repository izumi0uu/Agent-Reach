...
- Expanded the closed registry from 29 to 33 operations with Twitter post
  search, Xiaohongshu note search, Xueqiu stock search, and Exa Code search.
- Xueqiu receives only a one-attempt host capability, and Exa Code uses its
  dedicated endpoint and `query + numResults` contract.
